...
void main() {
  group('Authentication Flow Tests', () {
    test('Session creation stores data correctly', () async {
      ...
      final sessionId =
          await DatabaseHelper.instance.createSession('Test Chat');
    ...
    test('Messages are stored in session', () async {
      // Create test session
      final sessionId =
          await DatabaseHelper.instance.createSession('Test Session');
    ...
    test('Session deletion removes all data', () async {
      ...
      final sessionId =
          await DatabaseHelper.instance.createSession('Delete Test');
  ...
  group('Message Pagination Tests', () {
    ...
    setUp(() async {
      ...
      testSessionId =
          await DatabaseHelper.instance.createSession('Pagination Test');
      for (int i = 0; i < 100; i++) {
        await DatabaseHelper.instance.createMessage(
          testSessionId,
  ...
  group('Session Management Tests', () {
    test('Multiple sessions can coexist', () async {
      ...
      await DatabaseHelper.instance
          .createMessage(session1, 'user', 'Message in 1');
      await DatabaseHelper.instance
          .createMessage(session2, 'user', 'Message in 2');
    ...
    test('Session title can be updated', () async {
      final sessionId =
          await DatabaseHelper.instance.createSession('Original Title');
